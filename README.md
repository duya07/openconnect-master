...
`/usr/local/sbin/oc-master` 是安装后的持久程序本体，systemd 服务和独立回滚任务都会调用它；`/usr/local/bin/ocm` 只是便于输入的快捷链接。
...
每次启动会把选中的完整账户记录固化到本次运行快照中；之后即使账户列表重新排序，当前连接也不会改为读取同一索引下的其他账户。下一次显式启动仍以当时的账户列表和选择为准。
...
全局模式只在能够唯一证明原出口和入站回程时启动。若检测到多个非 VPN 默认出口、ECMP、多 WAN 或无法安全解析的现有策略路由，会在写入网络配置前直接拒绝；这类主机应优先使用本地 SOCKS5 模式，或先自行简化路由拓扑。
...
4. **日志提示 `CLEANUP_FAILED` 或清理未完成**
   - 先执行 `sudo ocm stop` 重试显式恢复，不要直接启动另一个模式。
   - 若仍失败，使用 `sudo ocm logs` 查看未能清理的项目资源；脚本会保留恢复证据并拒绝新启动，直到能够证明清理完成。
