...
### 已知限制：iptables 备用转发在 NAT 型 VPS 上不可用

socat 缺失时 Netns 模式会退到 iptables DNAT 转发。该备用方案在 **NAT 型 VPS**（网卡是内网地址、公网 IP 由上游网关 NAT）上数据面不通，原因有三（实测）：

- 本机经 `127.0.0.1` 连接：DNAT 后源地址 `127.0.0.1` 的包进入 veth 会被 netns 内核按 martian 丢弃（Linux 不允许 127/8 源出现在非 lo 接口）；
- 本机经公网 IP 连接：公网 IP 不是本机地址，包直接发往网关被 RST；
- 外部客户端连接：需要网关把对应端口映射到本机（默认没有）。

其中第一条与 NAT 无关——127/8 源出现在非 lo 接口被丢弃是内核硬约束，**任何机器上**（包括公网 IP 直挂网卡的）本机 127 访问都不通。公网 IP 直挂网卡的机器上，"外部客户端连接"与"本机经公网 IP 连接"两条路径理论上可用（DNAT 规则与回程 MASQUERADE 形状正确），但未在真实环境实测。在这类机器上，本机使用请仍走公网 IP 而非 127.0.0.1。
