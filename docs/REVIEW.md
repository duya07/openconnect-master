...
验证机为 NMCloud-Hinet 的 Debian x86_64（Linux 6.10.10，约 974 MiB RAM，无 swap）。候选源码以归档上传到 `/tmp/oc-master-verify-20260913-9f667c9-step8`，归档 SHA-256 为 `5e03094aca10c2a7614b29bf58023a6bb38dbd801590ce5aa86402a81f8c4fda`。所有测试通过 `OCM_*` 使用该目录内的配置、运行时、unit、锁、命令桩和伪 `/proc` 路径，没有调用真实 `start-global`/`start-proxy`。
