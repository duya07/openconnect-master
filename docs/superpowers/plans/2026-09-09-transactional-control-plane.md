...
- 所有测试使用 `OCM_*` 路径覆盖和临时目录；不得在开发/验证中修改 NMCloud-Hinet 的真实 VPN、默认路由或生产 systemd 单元。
...
在 `docs/REVIEW.md` 追加本次架构问题、实现映射、测试命令、Linux 隔离测试结果、性能/RSS/状态大小和未施工候选；保留既有 live proof，不声称执行过 NMCloud-Hinet 真实 VPN/路由测试。
...
- [ ] **Step 6: 在 NMCloud-Hinet 仅做隔离 Linux 验证**
